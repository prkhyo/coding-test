...
# 리스트 자료형은 가장 오른쪽에 원소를 삽입하는 append() 메서드와 
# 가장 오른쪽에서부터 원소를 꺼내는 pop() 메서드를 지원
...
#            A       B
# 1단계     192     162
# 2단계     162      30    -> B인 '162'와 A를 B로 나눈 나머지인 '30'
# 3단계      30      12    -> B인 '30' 과 A를 B로 나눈 나머지인 '12'
# 4단계      12       6    -> B인 '12'와 A를 B로 나눈 나머지인  '6'

# => 192와 162의 최대공약수는 12와 6의 최대공약수와 같음
# 12는 6의 배수이기 때문에 6이 최대공약수가 됨

# 더 작은 수를 이용해서 식을 간단하게 바꾸는 형태가 반복적이며 동일한 구조를 가지고 있기 때문에
# 재귀함수를 이용해서 이러한 과정을 직관적으로 코드로 옮길 수 있음
...
#   더 좋은 방법을 고려한 뒤 적용할 것
# 컴퓨터가 함수를 연속적으로 호출하면 컴퓨터 메모리 내부의 스택 프레임에 쌓임
#   그래서 스택을 사용해야 할 때 구현상 스택 라이브러리 대신에 재귀 함수를 이용하는 경우가 많음
#   (ex) DFS를 더 간결한 코드로 구현하기 위해 재귀 함수를 사용하기도 함
